...
function Home() {
    ...
    const registerUser = async () => {
      try {
        if (queryUsername || queryUserId) {
          const response = await axios.post("https://new-server-one-alpha.vercel.app/api/user/register", {
  ...
  const toggleFavorite = async (propertyId) => {
    try {
      ...
      if (isLiked) {
        ...
        await axios.delete(`https://new-server-one-alpha.vercel.app/api/user/dislikes/${propertyId}`, {
          data: { email },
        });
        setFavorites((prev) => prev.filter((id) => id !== propertyId));
      } else {
        // Send a POST request to add the like
        await axios.post(`https://new-server-one-alpha.vercel.app/api/user/likes/${propertyId}`, { email });
  ...
  const Write = async (property) => {
    ...
    try {
      const response = await axios.post(`https://new-server-one-alpha.vercel.app/api/user/addInterest/${property.id}`, {
        teleNumber,
      });
